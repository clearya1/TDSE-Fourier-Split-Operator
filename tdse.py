...
if rank==0:
    f = h5py.File("pde.hdf5", "a")
    grp = f["space"]
    #del grp["N_"+str(N)+"Range_"+str(xMax)]
    grp1 = grp.create_group("N_"+str(N)+"Range_"+str(xMax))

    grp2 = f["momentum"]
    #del grp2["N_"+str(N)+"Range_"+str(xMax)]
    grp3 = grp2.create_group("N_"+str(N)+"Range_"+str(xMax))
    
    label = "0"                             #whatever you want this run to be called
                                            #and saved as in pde.hdf5
start = time.time()
...
#u[:] = np.ones(u.shape).astype(u.dtype)              #initial conditions
#u[:] = np.random.rand(*u.shape).astype(u.dtype)      #initial conditions
...
if rank==0:
    ...
    dset = grp1.create_dataset(label, data=groundState)
    dset.attrs['time'] = end-start
    
    dset = grp3.create_dataset(label, data=groundState)
    dset.attrs['time'] = end-start
